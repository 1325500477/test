...
static unsigned int app_port=80; // 80 is just a default value. Gets overwritten during init
static unsigned char macaddr[6];
static unsigned char ipaddr[4];
static signed int info_hdr_len=0;
static signed int info_data_len=0;
...
unsigned int checksum(unsigned char *buf, unsigned int len,unsigned char type){
        ...
        while(len >1){
                sum += 0xFFFF & (((unsigned long)*buf<<8)|*(buf+1));
                buf+=2;
                len-=2;
        }
        // if there is a byte left then add it (padded with zero)
        if (len){
                sum += ((unsigned long)(0xFF & *buf))<<8;
        }
...
void init_ip_arp_udp_tcp(unsigned char *mymac,unsigned char *myip,unsigned int tmpport){
        unsigned char i=0;
        app_port=tmpport;
        while(i<4){
                ipaddr[i]=myip[i];
...
unsigned char eth_type_is_my_mac(unsigned char *buf,unsigned int len)
{
    unsigned char i = 0;
    if(len < 12)
      return 0;
...
unsigned char eth_type_is_boardcast(unsigned char *buf,unsigned int len)
{
    unsigned char i = 0;
    if(len < 12)
      return 0;
...
void make_tcphead(unsigned char *buf,unsigned int rel_ack_num,unsigned char mss,unsigned char cp_seq)
{
        ...
        buf[TCP_SRC_PORT_H_P] = app_port >> 8;
        buf[TCP_SRC_PORT_L_P] = app_port & 0xFF;
        i=4;
        // sequence numbers:
...
// you can send a max of 1472 bytes of data
void make_udp_reply_from_request(unsigned char *buf,char *data,unsigned int datalen,unsigned int port)
{
		...
		len_temp = IP_HEADER_LEN+UDP_HEADER_LEN+datalen;   	//一共最多运行1500字节
        buf[IP_TOTLEN_H_P]=len_temp>>8;
        buf[IP_TOTLEN_L_P]=len_temp&0xff;
        make_ip(buf);
        // send to port:
		...
		len_temp = UDP_HEADER_LEN+datalen;
        buf[UDP_LEN_H_P]=len_temp>>8;
        buf[UDP_LEN_L_P]=len_temp&0xff;
        // zero the checksum
...
{
        info_data_len=(((signed int)buf[IP_TOTLEN_H_P])<<8)|(buf[IP_TOTLEN_L_P]&0xff);
...
//unsigned int fill_tcp_data_p(unsigned char *buf,unsigned int pos, const prog_char *progmem_s)
...
void make_winpcap_ack_with_data(unsigned char *buf,unsigned int dlen)
{
        // fill the mac:
        make_eth(buf);
        enc28j60PacketSend(12 + dlen,buf);
}
/* end of ip_arp_udp.c */
